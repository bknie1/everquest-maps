...
import sys, math
from eqmap_toolkit import Canvas, frame, title, compass, title_band_knockout, flame_motif
...
THEMES={
 ...
 'fire':   dict(OUT=(96,50,28), INN=(216,92,30), GC=(168,120,90), TC=(184,70,22), SH=(216,92,30)),
 'dragon': dict(OUT=(92,34,28), INN=(200,60,40), GC=(170,110,80), TC=(168,44,30), SH=(200,60,40)),    # stonebrunt
}
T=THEMES[THEME]
def parse(path):
    L=[]
...
_ctr = flame_motif if THEME in ('fire','dragon') else None
compass(cv, cv.bx0+bc, cv.by1-bc, CR, ring=(T['OUT'],T['INN']), rose=(T['INN'],T['GC']), center=_ctr, center_colors=(T['INN'],T['GC']), label=T['OUT'], n_label=T['TC'], arrow=T['TC'])
