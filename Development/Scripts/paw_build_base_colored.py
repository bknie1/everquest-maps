...
def add_river_f1():
    # Floor-1 ravine: a little river running west into the CENTRAL CHASM, traced from
    # in-game /loc points converted to native the same way markers are: native=(-b,-a).
    z=0.0
    center=[(26,-8),(23,-22),(7,-27),(-13,-31),(-30,-33),(-46,-33)]   # east ledge -> chasm
    wm=np.zeros((gh,gw),bool)
    for (x1,y1),(x2,y2) in zip(center,center[1:]):
        n=int(max(abs(x2-x1),abs(y2-y1))/CELL)+1
        for i in range(n+1):
            t=i/n; stamp_disc(wm, x1+(x2-x1)*t, y1+(y2-y1)*t, 6.5)   # little river
    # central chasm pool (the teal pool the river pours into)
    stamp_disc(wm, -26, -33, 20); stamp_disc(wm, -12, -31, 13)
    ncells=fill_water_mask(wm, z, 'f1river')
    # a few chevrons at the chasm showing the river pouring in (west edge)
    for yy in (-40,-33,-26):
        L(-40, yy, -46, yy-4, WRIP, z); L(-46, yy-4, -52, yy, WRIP, z)
    return ncells
