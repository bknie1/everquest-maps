# soldungc

**Title:** CAVERNS OF EXILE (16 chars)
**Title style:** unreviewed
**Title bbox:** x[-1360,1298] y[-1536,-992] (h 543)
**Title inks:** (90, 62, 46) x759, (116, 84, 64) x232, (112, 60, 36) x100, (158, 62, 28) x95
**Frame width:** 2463
**Compass:** 1 rose(s), ring 24px at 900px fit
**Layers:** _1=0, _2=8118, base=22825
**Total strokes:** 30943 (budget 31000) | POIs 6 | dupes 0 | inks 13
**eqqms:** overall A (format A, budget A, title A, dupes A, palette A)

## Notes

(none yet)
2026-09-12 (final sweep): title scaled 0.80 in place (ink 162,70,34); compass rebuilt at the standard size (ring 24px at a 900px fit, restd_compass.py).
